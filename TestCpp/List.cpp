...
public:
	...
	bool TryGetValueAtIndex(const size_t index, bool& outIsSuccess) const noexcept //pobranie wartoœci pod indeksem, outIsSuccess true je¿eli operacja powiod³a siê, flase w przeciwnym wypadku
	{
	...
	bool TryRemoveValue() noexcept //usuwa ostatni element z listy
	{
		if (length > 0)
		{
			TryMoveCurrentIndex(length - 1);
...
void PrintArray(const DoubleLinkedList<T>& list) noexcept //wyœwietla listê
{
	...
	{
		value = list.GetValueAtIndex(i, isSucceed);
		if (!isSucceed)
		{
			return false;
		}
